...
static inline int ds2780_read8(struct ds2780_device_info *dev, u8 *val, int addr)
{
	return ds2780_battery_io(dev, val, addr, sizeof(u8), 0);
}

static int ds2780_read16(struct ds2780_device_info *dev, s16 *val, int addr)
{
	int ret;
	u8 raw[2];

	ret = ds2780_battery_io(dev, raw, addr, sizeof(raw), 0);
	if (ret < 0)
		return ret;
...
static inline int ds2780_read_block(struct ds2780_device_info *dev, u8 *val, int addr,
	size_t count)
{
	return ds2780_battery_io(dev, val, addr, count, 0);
}

static inline int ds2780_write(struct ds2780_device_info *dev, u8 *val, int addr,
	size_t count)
{
	return ds2780_battery_io(dev, val, addr, count, 1);
}
